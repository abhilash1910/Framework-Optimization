...
from parallel_config import ModelParallelConfig
...
def divide(num,den):
    assert num%den==0, "{} is not divisible by {}".format(num, den)
    return num//den
...
def _initialize_affine_weight_cpu(weight, output_size, input_size,
    ...
    my_weight_list = weight_list[global_rank::world_size]
...
class LinearWithGradAccumulationAndAsyncCommunication(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        ...
        if ctx.async_grad_allreduce:
            ...
            handle = dist.all_reduce(grad_input, group=get_tensor_model_parallel_group(), async_op=True)
